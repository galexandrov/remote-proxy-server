...
public class AndroidJsDebugger implements Debugger
{
	...
	private WebSocketServer debugWebSocketServer;

	private final static  String STOP_MESSAGE = "STOP_MESSAGE";

	private byte[] LINE_END_BYTES;

	private DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:sssZ");
	...
	private class RequestHandler implements Runnable
	{
		...
		public RequestHandler(Socket socket, Closeable responseHandlerCloseable) throws IOException
		{
		...
		public void run()
		{
			...
			try
			{
				...
				{
					Log.d("TNS.JAVA.JsDebugger", "SOCKET_LINE ["+ dateFormat.format(new java.util.Date()) + "] Inspector -> Mobile: " + line.toString());
					...
					switch (state)
					{
						...
						case Message:
							if ((-1 < messageLength) && (messageLength <= line.length()))
							{
								...
								//Log.d("TNS.JAVA.JsDebugger", "["+ dateFormat.format(new java.util.Date()) + "] Inspector -> Mobile: " + message);
	...
	private class ResponseHandler implements Runnable
	{
		...
		public ResponseHandler(Socket socket, Closeable requestHandlerCloseable) throws IOException
		{
		...
		public void run()
		{
			...
			this.sendMessageToInspector("{\"handshake-key\":\"xervca04kx2mgja5d4jme59a8\"}");
		...
		private void sendMessageToInspector(String msg)
		{
			...
			if (utf8 != null)
			{
				try
				{
					...
					Log.d("TNS.JAVA.JsDebugger", "["+ dateFormat.format(new java.util.Date()) + "] Inspector <- Mobile :" + msg);
				}
				catch (IOException e)
				{
					e.printStackTrace();
